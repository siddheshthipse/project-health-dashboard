...
const RiskHeatmap = () => {
    ...
    const handleCellClick = (impact, probability) => {
        const cell = riskData.cells.find(
        ...
        <div className="bg-white rounded-lg shadow p-5 border-t-4 border-blue-500">
            ...
            <div className="flex mb-4">
                ...
                <div className="flex flex-col mr-1">
                    <div className="h-8 flex items-center justify-center font-semibold text-sm bg-blue-100 rounded-t text-blue-800">
                        TOTAL
                    </div>
                    ...
                    <div className="h-8 w-12 bg-gray-300 flex items-center justify-center font-semibold text-gray-800">
                        {riskData.totals.all}
                    </div>
                </div>
                ...
                <div className="flex-1">
                    {/* Header */}
                    <div className="bg-blue-500 h-8 text-white font-semibold text-sm flex items-center justify-center rounded-t">
                        Risk Heatmap
                    </div>
                    ...
                    <div className="flex">
                        ...
                        <div className="flex flex-col">
                            <div className="h-30 w-8 bg-gray-200 flex flex-col items-center justify-center">
                                {/* <div className="transform -rotate-90 w-30 text-center font-medium text-xs text-gray-700">
                                    Impact
                                </div> */}
                            </div>
                            <div className={`h-10 w-8 ${getImpactRowColor('high')} flex items-center justify-center`}>
                                <span className="transform -rotate-90 font-medium text-xs text-white">High</span>
                            </div>
                            <div className={`h-10 w-8 ${getImpactRowColor('medium')} flex items-center justify-center`}>
                                <span className="transform -rotate-90 font-medium text-xs text-gray-800">Medium</span>
                            </div>
                            <div className={`h-10 w-8 ${getImpactRowColor('low')} flex items-center justify-center`}>
                                <span className="transform -rotate-90 font-medium text-xs text-gray-800">Low</span>
                            </div>
                        </div>
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex h-8 bg-gray-200">
                                <div className="flex-1 flex items-center justify-center font-medium text-xs text-gray-700">Probability</div>
                            </div>

                            {/* Probability level markers */}
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Planned To</th>
                                        </tr>
                                    </thead>
                                            ...
                                            <tr key={index} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                                        ...
                                                        risk.status === 'Pending' ? 'bg-purple-100 text-purple-800' :
                                                        risk.status === 'Scheduled' ? 'bg-indigo-100 text-indigo-800' :
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                                        risk.impactScore === 'Critical' ? 'bg-red-100 text-red-800' :
                                                        risk.impactScore === 'High' ? 'bg-orange-100 text-orange-800' :
                                                        risk.impactScore === 'Medium' ? 'bg-yellow-100 text-yellow-800' :
                                                        risk.impactScore === 'Low' ? 'bg-green-100 text-green-800' :
                                                        'bg-gray-100 text-gray-800'
                                                    }`}>
                                                        {risk.impactScore}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{risk.responsiblePerson}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{risk.createdBy}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{risk.plannedTo}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
